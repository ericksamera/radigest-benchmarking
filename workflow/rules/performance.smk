...
LARGE_GENOME_SUMMARY = "results/performance/large_genome/large_genome_summary.tsv"
...
SCREENING_SPEED_CASE_BY_ID = {row["case_id"]: row for row in SCREENING_SPEED_CASE_ROWS}
...
THREAD_SCALING_CASE_BY_ID = {row["case_id"]: row for row in THREAD_SCALING_CASE_ROWS}
...
rule run_radigest_screening_speed_case:
    ...
    conda:
        "../envs/benchmark.yml"
    threads: lambda wc: max(screening_int(wc, "jobs"), screening_int(wc, "radigest_threads"))
    params:
...
rule run_radigest_thread_scaling_case:
    ...
    conda:
        "../envs/benchmark.yml"
    threads: lambda wc: thread_scaling_int(wc, "threads")
    params:
        radigest=lambda wildcards: config.get("radigest", "radigest"),
...
rule run_radigest_pair_screen_scaling_case:
    ...
    conda:
        "../envs/benchmark.yml"
    threads: lambda wc: pair_screen_scaling_int(wc, "jobs")
    resources:
...
rule run_radigest_large_genome_case:
    ...
    conda:
        "../envs/benchmark.yml"
    threads: lambda wc: large_genome_int(wc, "threads")
    resources:
...
rule run_matched_tool_timing_radigest:
    ...
    params:
        tool_id="radigest",
        radigest=lambda wildcards: config.get("radigest", "radigest"),
        ddgrader_repo=lambda wildcards: config.get(
