...
def read_lengths(path: Path) -> list[int]:
    ...
    with path.open(newline="", encoding="utf-8") as handle:
        ...
        for row in reader:
            hard_kept = row.get("hard_kept", row.get("hard_kept?", ""))
            if hard_kept and hard_kept.lower() not in {"true", "1", "yes"}:
                continue
...
def write_summary(
    ...
    in_window = [
        row
        for row in rows
        if int(row["bin_upper"]) >= min_size and int(row["bin_upper"]) <= max_size
    ]
        ...
        [
            {
                ...
                "notes": (
                    ...
                    "bin_upper is used for approximate base totals"
                ),
            }
        ],
        SUMMARY_COLUMNS,
