#!/usr/bin/env python3
"""Summarize radigest per-run timing rows for performance claims."""
...
def read_case_rows(path: Path, category: str) -> dict[str, dict[str, str]]:
    ...
    if not selected:
        fail(f"{path}: no required rows for category={category!r}")
    return selected
...
def summarize_case(
    ...
    configured_runs = parse_int(case["runs"], path_label=f"case {case['case_id']} runs")
    successes = [row for row in run_rows if row.get("status") == "PASS"]
    durations = [
        parse_float(
            row["wall_seconds"],
            path_label=f"case {case['case_id']} wall_seconds",
        )
    ...
    retained_counts = [
        parse_int(
            ...
            path_label=f"case {case['case_id']} retained_fragments",
        )
        for row in successes
    ]
    ...
    return {
        ...
        "output_mode": successes[0]["output_mode"] if successes else "NA",
        ...
        "relative_to_fastest_median": "NA",
        "status": status,
        "notes": case["notes"],
    }


def add_group_consistency_and_relative_timings(rows: list[dict[str, str]]) -> None:
    ...
    for group_rows in grouped.values():
        ...
        if len(retained_values) != 1:
            for row in group_rows:
                ...
                row["notes"] = (
                    row["notes"]
                    + " Group retained-fragment counts differ across "
                    + "input formats."
                )

        medians = [
            float(row["wall_seconds_median"])
            for row in group_rows
            if row["status"] == "PASS" and row["wall_seconds_median"] != "NA"
        ]
        if not medians:
            continue
        fastest = min(medians)
        for row in group_rows:
            if row["wall_seconds_median"] == "NA":
                continue
            row["relative_to_fastest_median"] = fmt_float(
                float(row["wall_seconds_median"]) / fastest
            )
...
def main() -> int:
    args = parse_args()
    case_by_id = read_case_rows(args.cases, args.category)
    ...
    add_group_consistency_and_relative_timings(summaries)
    write_rows(args.out, summaries)

    failed = [row for row in summaries if row["status"] != "PASS"]
    if args.require_pass and failed:
        print(
            f"{len(failed)} of {len(summaries)} performance summaries failed; "
            f"see {args.out}",
            file=sys.stderr,
        )
        return 1

    print(f"Wrote {len(summaries)} performance summary rows to {args.out}")
    return 0
