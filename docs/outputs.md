...
The honeysuckle target-driven design-space figure is generated from two radigest-design runs, one at 2.0% recovered genome and one at 1.5% recovered genome:

```text
results/experimental_design/honeysuckle_gca021464415_2pct_60x_20samples/design.tsv
results/experimental_design/honeysuckle_gca021464415_1p5pct_60x_20samples/design.tsv
results/manuscript/figures/figure_02_honeysuckle_design_space.pdf
```

These figures are generated by `make figures`, `make performance`, `make manuscript`, or the focused `make experimental-honeysuckle-design-space-figure` target. The matched-tool timing figure is promoted in `config/artifacts.tsv` because it is the figure-level view of release-required matched timing.
