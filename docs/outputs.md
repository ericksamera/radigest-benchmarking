...
These starter figures are generated by `make figures` or `make manuscript`. They should be promoted into `config/artifacts.tsv` only when the manuscript depends on a figure as claim evidence.
