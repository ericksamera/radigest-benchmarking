...
Stage 6 empirical scaffold outputs declare the tracked manifest contract, the
Sockeye reference download, and per-library BAM inventories for enabled rows:
...
Later empirical stages will add TLEN histograms, model-fit summaries, and
manuscript plots from enabled local BAM-directory rows.
