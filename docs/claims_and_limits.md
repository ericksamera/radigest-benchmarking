...
Stage 5e large-reference outputs support a radigest-only timing claim on the moderate public cannabis Pink Pepper reference. They do not support cross-tool equivalence, screening scaling, or empirical recovery claims. The required row uses JSON output to avoid turning the claim into a fragment-TSV disk-output stress test.
