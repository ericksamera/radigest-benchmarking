...
The input-format tier compares plain and gzip FASTA inputs for `small_yeast_s288c_B1` using `config/performance_cases.tsv`. The screening-speed tier measures native radigest candidate-pair screening on `small_yeast_s288c_plain` using `config/screening_speed_cases.tsv`. Later Stage 5 patches should add thread scaling, pair-screen scaling, and large-genome cases.
