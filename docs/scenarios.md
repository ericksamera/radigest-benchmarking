...
Stage 5a adds a performance input-format tier:
...
The current performance tier compares plain and gzip FASTA inputs for `small_yeast_s288c_B1` using `config/performance_cases.tsv`. Later Stage 5 patches should add screening speed, thread scaling, pair-screen scaling, and large-genome cases.
